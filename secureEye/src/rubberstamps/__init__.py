...
class RubberStamp:
    """SecureEye rubber stamp"""

    UI_TEXT = "ui_text"
    UI_SUBTEXT = "ui_subtext"

    def set_ui_text(self, text, type=None):
        """Convert an ui string to input secureEye-gtk understands"""
        typedec = "M"
    ...
    def send_ui_raw(self, command):
        """Write raw command to secureEye-gtk stdin"""
        if self.config.getboolean("debug", "verbose_stamps", fallback=False):
            print("Sending command to secureEye-gtk: " + command)
...
def execute(config, gtk_proc, opencv):  # noqa: C901
