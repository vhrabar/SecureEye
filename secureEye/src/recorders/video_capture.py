...
class VideoCapture:
    def __init__(self, config):
        ...
        if not os.path.exists(self.config.get("video", "device_path")):
            if self.config.getboolean("video", "warn_no_device", fallback=True):
                print(
                    _(
                        "SecureEye could not find a camera device at the path specified in the config file."
                    )
                )
                print(
                    _(
    ...
    def _create_reader(self):
        ...
        if recording_plugin == "ffmpeg":
            ...
            self.internal = cv2.VideoCapture(self.config.get("video", "device_path"), cv2.CAP_V4L)
