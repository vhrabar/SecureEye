...
const auto DEFAULT_TIMEOUT = std::chrono::duration<int, std::chrono::milliseconds::period>(100);
...
auto get_remaining_timeout_ms(const std::chrono::steady_clock::time_point& deadline) -> int {
...
auto connect_authd(const std::chrono::steady_clock::time_point& deadline) -> int {
    ...
    const auto connect_res = connect(sock_fd, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr));
    if (connect_res == 0) {
        return sock_fd;
    }
    ...
    if (getsockopt(sock_fd, SOL_SOCKET, SO_ERROR, &so_error, &so_error_len) != 0 || so_error != 0) {
        close(sock_fd);
        return -1;
    }
...
auto read_authd_response_code(const std::string& payload, const std::string& request_id, int& result_code) -> bool {
    ...
    if (!std::regex_search(payload, match, type_re) || match.size() < 2 || match[1].str() != "auth_response") {
        return false;
    }

    if (!std::regex_search(payload, match, request_id_re) || match.size() < 2 || match[1].str() != request_id) {
        return false;
    }
...
auto authd_authenticate(pam_handle_t* pamh, const std::string& username, int auth_timeout_ms) -> int {
    const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(auth_timeout_ms);
    ...
    const auto payload = R"({"v":)" + std::to_string(AUTH_PROTOCOL_VERSION) +
                         R"(,"type":"auth_request","request_id":")" + json_escape(request_id) + R"(","username":")" +
                         json_escape(username) + R"(","service":")" + json_escape(service) + R"(","tty":")" +
                         json_escape(tty) + R"(","rhost":")" + json_escape(rhost) + R"(","deadline_ms":)" +
    ...
    if (!send_all(sock_fd, reinterpret_cast<const char*>(&frame_len), sizeof(frame_len), deadline) ||
    ...
    if (!recv_exact(sock_fd, reinterpret_cast<char*>(&response_len_net), sizeof(response_len_net), deadline)) {
        close(sock_fd);
        return AUTHD_INTERNAL_ERROR;
    }
 ...
 */
auto secureEye_error(int status, const std::function<int(int, const char*)>& conv_function) -> int {
    switch (status) {
    case CompareError::NO_FACE_MODEL:
...
auto secureEye_status(char* username, int status, const INIReader& config,
    ...
    if (!config.GetBoolean("core", "no_confirmation", true)) {
        ...
        std::string identify_msg = confirm_text.replace(confirm_text.find("{}"), 2, std::string(username));
...
auto check_enabled(const INIReader& config, const char* username) -> int {
    ...
    if (config.GetBoolean("core", "abort_if_ssh", true)) {
        if (checkenv("SSH_CONNECTION") || checkenv("SSH_CLIENT") || checkenv("SSH_TTY") || checkenv("SSHD_OPTS")) {
 ...
 */
auto stop_password_prompt(optional_task<std::tuple<int, char*>>& pass_task, Workaround workaround,
    ...
    if (euidaccess("/dev/uinput", W_OK | R_OK) != 0) {
        ...
        conv_function(PAM_ERROR_MSG, S("Insufficient permissions to send Enter "
        ...
        try {
            ...
            for (retries = 0; retries < MAX_RETRIES && pass_task.wait(DEFAULT_TIMEOUT) == std::future_status::timeout;
                 retries++) {
                enter_device.send_enter_press();
            }
...
auto identify(pam_handle_t* pamh, int flags, int argc, const char** argv, bool ask_auth_tok) -> int {
    ...
    Workaround workaround = get_workaround(config.GetString("core", "workaround", "input"));
    ...
    const auto configured_timeout_ms = video_timeout_s > 0 ? static_cast<std::int64_t>(video_timeout_s) * 1000
                                                           : static_cast<std::int64_t>(AUTH_TIMEOUT_MS);
    const auto auth_timeout_ms = static_cast<int>(std::clamp<std::int64_t>(configured_timeout_ms + 1900, 1000, 30000));
    ...
    optional_task<std::tuple<int, char*>> pass_task([&]() -> std::tuple<int, char*> {
        ...
        int pam_res = pam_get_authtok(pamh, PAM_AUTHTOK, const_cast<const char**>(&auth_tok_ptr), nullptr);
        {
            std::unique_lock<std::mutex> lock(mutx);
            if (confirmation_type == ConfirmationType::Unset) {
    ...
    {
        std::unique_lock<std::mutex> lock(mutx);
        convar.wait(lock, [&]() -> bool { return confirmation_type != ConfirmationType::Unset; });
    }
...
// the sudo command
PAM_EXTERN auto pam_sm_authenticate(pam_handle_t* pamh, int flags, int argc, const char** argv) -> int {
    return identify(pamh, flags, argc, argv, true);
}

// Called by PAM when a session is started, such as by the su command
PAM_EXTERN auto pam_sm_open_session(pam_handle_t* pamh, int flags, int argc, const char** argv) -> int {
    return identify(pamh, flags, argc, argv, false);
}

// The functions below are required by PAM, but not needed in this module
PAM_EXTERN auto pam_sm_acct_mgmt(pam_handle_t* pamh, int flags, int argc, const char** argv) -> int {
    return PAM_IGNORE;
}
PAM_EXTERN auto pam_sm_close_session(pam_handle_t* pamh, int flags, int argc, const char** argv) -> int {
    return PAM_IGNORE;
}
PAM_EXTERN auto pam_sm_chauthtok(pam_handle_t* pamh, int flags, int argc, const char** argv) -> int {
    return PAM_IGNORE;
}
